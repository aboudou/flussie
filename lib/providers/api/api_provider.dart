...
class ApiProvider {
  ...
  Future<JsonProvider> get _jsonProvider async => await isDemo ? _jsonLocalProvider : _jsonNetworkProvider;
  Future<BinaryProvider> get _binaryProvider async => await isDemo ? _binaryLocalProvider : _binaryNetworkProvider;

  Future<List<VehicleListItem>?> getVehicles() async {
    final response = await (await _jsonProvider).fetchVehicles();
  ...
  Future<Uint8List> getMapImage(String vin, {int width = 100, int height = 100, int zoom = 13}) async {
    return await (await _binaryProvider).fetchMap(vin, width: width, height: height, zoom: zoom);
  }

  Future<Vehicle> getVehicle(String vin) async {
    final response = await (await _jsonProvider).fetchVehicle(vin);
  ...
  Future<Location> getLocation(String vin) async {
    final response = await (await _jsonProvider).fetchLocation(vin);
  ...
  Future<BatteryHealthList> getBatteryHealth() async {
    final response = await (await _jsonProvider).fetchBatteryHealth();
  ...
  Future<ChargeList> getCharges(String vin, bool superchargersOnly, int startDate, int endDate) async {
    final response = await (await _jsonProvider).fetchCharges(vin, superchargersOnly, startDate, endDate);
  ...
  Future<DriveList> getDrives(String vin, int startDate, int endDate) async {
    final response = await (await _jsonProvider).fetchDrives(vin, startDate, endDate);
  ...
  Future<Path> getPath(String vin, int startDate, int endDate) async {
    final response = await (await _jsonProvider).fetchPath(vin, startDate, endDate);
